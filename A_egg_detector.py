...
def batch(path,sf=1,s=1,gs=3,ksize=1):
    ...
    for i in os.listdir(image_folder):
        pos=ed.bad_egg(i,dic)# Comprobamos si en la foto hay huevos malos, en caso afrimativo nos devuelve una lista con las posiciones de lo contrario retorna un False. 
        ...
        for e in range(2,np.amax(labels)+1):
            labels,_= segment_eggs(imo,ksize=ksize)# // Volvemos a etquitar cada huevo que hay en la imagen dado que en cada iteracion esta informacion se pierde
            labels[labels!=e]=0;labels[labels==e]=255#// Creamos una mascara binaria para un huevo según el orden etiquetado
            labels= np.uint8(labels)#// Para poder aplicar ciertas operaciones debemos convertirlo al formato adecuado.
            ...
            x,y,z,n= robust_bbox_search(B,pout=10)#// Sobre el huevo aplicamos la funcion que nos crea cuadrado con el huevo dentro
            if x>20 and y>20: # Nos aseguramos que el huevo este dentro del cuadrodado, dado que en ocasiones por la posicion del huevo dentro de la imagen puede dar error.
                ...
                if black == False:
                    if pos != False :#// Comprobamos que la actual imagen contenga huevos malos
                        ...
                        os.chdir(directory)#// Guardamos la imagen del huevo en el directorio especificado.   
                        cv.imwrite("Bueno"+'_{}.jpg'.format(count),img_2)#// Antes de guardarlo le damos un nombre y un formato
                else: pass
            else: pass
            count+=1
    ##########################################################################################
